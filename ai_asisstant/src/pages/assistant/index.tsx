...
import { IconChevronLeft, IconPencil, IconUserPlus } from "@tabler/icons-react";
...
const Assistant: NextPage = () => {
  ...
  const [editingAssistant, setEditingAssistant] = useState<EditAssistant>();
  ...
  const saveAssistant = (data: EditAssistant) => {
    if (data.id) {
      ...
      const newAssistant = { ...data, id: Date.now().toString() };
  ...
  const onAddAssistant = () => {
    const newAssistant = {
      ...ASSISTANT_INIT[0],
      name: `助理_${assistantList.length + 1}`,
    };
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <Drawer opened={opened} onClose={drawerHandler.close} size="lg" position="right">
        {editingAssistant && (
          <AssistantConfig assistant={editingAssistant} save={saveAssistant} remove={removeAssistant} />
        )}
      </Drawer>
    </div>
  );
