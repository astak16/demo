...
from config import KEEP_RECENT, MAX_ITERATIONS, MEMORY_DIR, SYSTEM_PROMPT
import llm
import prompts
import tools
...
def run(user_input: str, messages: list[dict]):
    """
    执行一轮完整的 agent 交互
    ...
    for _ in range(MAX_ITERATIONS):
        ...
        if not parsed_calls:
            ...
            if _has_malformed_tool_call(response.content):
                ...
                messages.append(
                    {"role": "user", "content": prompts.TOOL_CALL_RETRY_HINT}
                )
                console.print("\n[dim](工具调用格式错误，要求模型重试)[/dim]")
                continue
            if response.content:
                console.print()
            messages.append({"role": "assistant", "content": response.content})
        ...
        for tc in parsed_calls:
            ...
            messages.append(
                {"role": "user", "content": prompts.format_tool_result(tc.name, result)}
            )
    return "[达到最大迭代次数]"
...
def _compact_if_needed(messages: list[dict]):
    ...
    for i in range(boundary):
        ...
        # 压缩工具结果消息（包含<tool_result> 标签且内容较长的消息）
        if "<tool_result>" in content and len(content) > 200:
            # 提取工具名，保留摘要
            # msg["content"] = "<tool_result>... (已压缩)"
...
def _parse_tool_calls(text: str) -> list[llm.ToolCall]:
    """
    从模型输出文本中解析 <tool_call>...</tool_call> 标签
    """

    # 先去掉 <thinking>...</thinking> 标签及其内容，避免干扰工具调用解析
    ...
    for match in matches:
        # 处理嵌套表情：只去掉纯容器标签(tool_call/glocall/request)
        inner = re.sub(
            r"</?(?:tool_call|glocall|request|tool_result)>", "", match
        ).strip()
...
def _try_parse_tool_json(raw: str) -> dict | None:
    ...
    stripped = re.sub(
        r"</?(?:gloss|gl|glm|gloc|globl|globbing|disabled|insert|loc|file_content)>",
        "",
        stripped,
    ).strip()
    ...
    name_match = re.search(
        r"<(?:name|command|tool_name)>\s*(.*?)\s*</(?:name|command|tool_name)>",
        stripped,
    )
    args_match = re.search(
        r"<(?:arguments|args)>\s*(.*?)\s*</(?:arguments|args)>", stripped, re.DOTALL
    )
        ...
        try:
            ...
            try:
                ...
                for pm in re.finditer(
                    r"<(\w+)>(.*?)</\1>", args_match.group(1), re.DOTALL
                ):
        ...
        if m:
            ...
            try:
                ...
                try:
                    ...
                    am = re.search(
                        r"<(?:arguments|args)>\s*(.*?)\s*</(?:arguments|args)>",
                        inner,
                        re.DOTALL,
                    )
                    if am:
                ...
                try:
                    ...
                    if am:
                        try:
                            ...
                            pass
    # 策略5 自闭和，XML 属性格式<file_read path="xxx/>
    ...
    if self_close:
        ...
        if real_name in tool_names:
            ...
            if not args:
                ...
                if eq_match:
                    args[eq_match.group(1)] = eq_match.group(2).strip("/")
            if args:
                return {"name": real_name, "arguments": args}
    return None
...
def _has_malformed_tool_call(text: str) -> bool:
    ...
    if re.search(
        r"<(?:tool_call|gloss|glocall|globs|globbing|glm|gloc|globl|request|disabled)\b",
        text,
    ):
        return True
    malformed_pattern = [
    ...
    return any(re.search(p, text, re.DOTALL) for p in malformed_pattern)
