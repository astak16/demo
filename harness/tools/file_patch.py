                ...
                "operations": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "end": {
                                "type": "integer",
                                "description": "结束行号（含），仅 delete 和 replace 需要",
                            },
                            "content": {
                                "type": "string",
                                "description": "要插入或替换的内容（多行用换行分隔），仅 insert 和 replace 需要",
                            },
                        },
...
def handle(path: str, operations: list[dict]) -> str:
    ...
    for op in ops:
        ...
        if si < 0 or ei > len(lines):
            return f"错误：操作行号超出范围（文件共 {len(lines)} 行），操作指定行号：{start}-{end}"
        if action == "delete":
            del lines[si:ei]
            changes.append(f"删除了第 {start} 行到第 {end} 行")
        elif action == "insert":
            new_lines = _to_lines(content)
            lines[si:si] = new_lines
            changes.append(f"在第 {start} 行后插入了 {len(new_lines)} 行")
        elif action == "replace":
            new_lines = _to_lines(content)
            lines[si:ei] = new_lines
            changes.append(f"替换了第 {start} 行到第 {end} 行，共 {len(new_lines)} 行")
