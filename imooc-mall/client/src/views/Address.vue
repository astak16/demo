<template>
    <div>
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  ...
                  <li class="addr-new" @click="openAddAddress">
      ...
      <modal :mdShow="isAddAddressShow" @close="closeAddAddress">
        <div slot="message" class="address-form">
          <h3>添加收货地址</h3>
          <input type="text" v-model.trim="addressForm.userName" placeholder="收件人姓名">
          <input type="text" v-model.trim="addressForm.streetName" placeholder="详细地址">
          <input type="text" v-model.trim="addressForm.postCode" placeholder="邮政编码">
          <input type="text" v-model.trim="addressForm.tel" placeholder="联系电话">
          <label class="address-default">
            <input type="checkbox" v-model="addressForm.isDefault">
            设为默认地址
          </label>
          <p class="address-form-error" v-if="addAddressError">{{addAddressError}}</p>
        </div>
        <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="addAddress">{{isAddingAddress ? '提交中...' : '确认添加'}}</a>
          <a class="btn btn--m btn--red" href="javascript:;" @click="closeAddAddress">取消</a>
        </div>
      </modal>
  ...
  export default{
      ...
      methods:{
          init(selectLast){
              axios.get("/users/addressList").then((response)=>{
                  ...
                  if(this.addressList.length){
                    let selectedIndex = selectLast ? this.addressList.length - 1 : 0;
                    this.selectedAddrId = this.addressList[selectedIndex].addressId;
          ...
          openAddAddress(){
            ...
            this.isAddAddressShow = true;
          },
          closeAddAddress(){
            if(this.isAddingAddress){
              return;
            }
            this.isAddAddressShow = false;
            this.addAddressError = '';
          },
          resetAddressForm(){
            this.addressForm = {
          ...
          addAddress(){
            ...
            if(!this.addressForm.userName || !this.addressForm.streetName ||
              !this.addressForm.postCode || !this.addressForm.tel){
              this.addAddressError = '请完整填写收货地址信息';
              return;
            }

            this.isAddingAddress = true;
            this.addAddressError = '';
            axios.post('/users/addAddress', this.addressForm).then((response)=>{
              let res = response.data;
              if(res.status=='0'){
                this.isAddAddressShow = false;
                ...
                this.addAddressError = res.msg || '添加地址失败';
              }
            }).catch(()=>{
              this.addAddressError = '网络异常，请稍后重试';
            }).then(()=>{
